...
Every complete durable record supplied to Task 2 is rechecked with `verifyRecordDigest()` before any semantic derivation. Digest equality alone is insufficient: consumed Task 2 qualification and eligibility records must also have `record_version: 1`, `lifecycle_state: active`, payload `record_mode: development_fixture`, payload `authority_effect: none`, exact scope/state/reason coherence, and the complete exact provenance relationships required by section 10. In particular, a qualified record has a decisive A/B outcome and the single qualified reason with both proofs true; an eligible record has the single eligible reason and a non-null permission ref. Re-finalizing a caller-mutated upstream record must not make it consumable. Every reference made from one input to another must equal the referenced record's exact `record_id`, `schema_id`, `schema_version`, and `content_digest`.
...
| `payload.evidence_reviewed` | event payload's already unique, nonempty set in the legacy function's historical JavaScript UTF-16 `.sort()` order |
| `payload.decided_at` | event `occurred_at` |

The adapter must preserve `recordContentDecision()` byte-for-byte, including its historical JavaScript UTF-16 ordering of `evidence_reviewed`; changing the legacy sorter is prohibited. It validates that historical order at the event boundary. A new durable representation may derive a separately canonicalized set only when the durable schema declares that order and the boundary continues to bind the unchanged event digest.
...
The pairwise-review snapshot is the immutable pre-adjudication review. It never contains an adjudication ref or the state `adjudicated`; this makes the evidence graph acyclic. A later adjudication snapshot points one way to that already-finalized review through `AdjudicationPayload.review_ref`. The reviewer-set snapshot contains participants only, never merely eligible nonparticipants. Reviewer identity is the stable `reviewer_ref.record_id`, not the full versioned `DigestRef`: a second version or digest for the same record ID is still the same participant, cannot count twice, and cannot evade candidate-author independence checks. With no adjudication, the pairwise review's reviewer refs must equal both the reviewer-set entry refs and the blinding-proof reviewer refs by stable record ID, with exact supplied versions still digest-verified. With valid adjudication, the stable-record-ID union of pairwise-review reviewer refs and the one adjudicator ref must equal both the reviewer-set entry refs and the blinding-proof reviewer refs. Thus an adjudicator whose disposition becomes the effective outcome must be covered by the same passing presentation-blinding proof. None may share a stable record ID with either candidate's author refs. Every entry must carry nonempty independence evidence and `independent_of_candidate_authorship: true` before it counts toward policy minima. Eligibility counts this exact stable-identity participant set.
...
Canonical collection semantics are also exact. Producer source/code artifacts are sorted by their `path` string, as required by the code-manifest preimage. Proposal alternatives and the two A/B order tuples are ordered. The legacy event payload's `evidence_reviewed` array is the sole compatibility exception and retains the historical JavaScript UTF-16 ordering defined in section 4. The rubric's allowed outcomes, stable dimensions, required checks, and allowed lineage classes retain their literal declared tuple order. All other semantically set-valued arrays—source/evidence refs, stable-set items, authors, reviewers, reviewer entries and scopes, independence evidence, lineage nodes and parents, rationale codes, policy scopes and transitions, permission project/subject refs, check evidence/reasons, and provenance—are unique and sorted by canonical JSON bytes. Reason codes use the state-specific ordering rules above.
...
No other `task2_contract_invalid:` suffix is emitted. The list is a closed vocabulary, not a global priority ranking across independent corruptions. A constructor first performs one descriptor-only whole-graph canonical-safety pass; an accessor, inherited value, symbol, hole, non-enumerable extra, cycle, `undefined`, function, bigint, non-finite number, or negative zero emits `canonical_value`, and getters are never invoked. It then validates nodes in the exact field order of the corresponding input interface, depth first and left to right. Within each node it validates required/unknown keys and cardinality, timestamps, IDs/discriminators, digest syntax and recomputation, then references and semantics. A snapshot-kind discriminator mismatch is `input_shape`; a valid discriminator paired with a wrong referenced schema ID is `schema_id`. When independent faults coexist, the first fault encountered by this frozen traversal wins. Implementations need not scan or retain every fault in an already-invalid graph merely to choose a diagnostic category. Later semantic enum mismatches explicitly assigned a substantive state in sections 7–9 remain valid-shaped evidence and are not collapsed into the structural pass.
