...
Task 4 adds only named exports from packages/learning/src/index.ts. The exact eight public runtime functions are:
...
The closed interfaces and type aliases declared by this contract are type-only package-root exports. No Task 4 validator, parser, resolver, cache, canonicalization helper, Unicode helper, feature-order module constant, internal verification result, or error-construction helper is a package-root runtime export.
...
Artifact paths are workspace-relative POSIX paths. Raw digest is sha256 over exact UTF-8 bytes. resolution_artifacts is the exact path-sorted set package.json, packages/core/package.json, packages/core/tsconfig.json, packages/learning/package.json, packages/learning/tsconfig.json, pnpm-lock.yaml, tsconfig.base.json, and tsconfig.json. The root package manifest binds the supported build, typecheck, test, lint, foundation, portable-runtime, voice-research, deterministic learning-fixture generation, and independent learning-verification commands. packages/learning/package.json declares exactly `"typescript-compiler": "npm:typescript@5.9.3"`; pnpm-lock.yaml resolves that alias to TypeScript 5.9.3. Those supplied, raw-byte-bound resolution artifacts make the compiler identity and public verification surface part of the producer witness. The compiler package is the trusted parser/resolver implementation and terminates dependency traversal like a Node built-in; it is not silently added to source_artifacts.

source_artifacts is the unique path-sorted exact transitive behavior-affecting module closure, not merely the entry paths. Starting at the table's entry paths, the declared resolution profile parses every supplied UTF-8 module with the aliased TypeScript 5.9.3 compiler API, rejects every parse diagnostic, and uses TypeScript NodeNext module resolution over only the supplied source and resolution bytes. It follows every non-type static import, non-type re-export, and dynamic import whose argument is an exact string literal. Escaped string literals are decoded by the compiler grammar before resolution. Relative imports and workspace-package exports resolve only under the supplied resolution_artifacts bytes; node: built-ins and the identity-bound compiler alias terminate traversal; computed or interpolated dynamic imports, require(), unresolved specifiers, path aliases without supplied resolution metadata, and runtime imports outside the supplied closure fail. Every identifier named eval, Function, AsyncFunction, GeneratorFunction, or AsyncGeneratorFunction fails. A property access named constructor fails. For an element access, the compiler-folded property name includes string and no-substitution-template literals, parentheses, template substitutions that are themselves statically string-foldable, and recursive string + concatenation; a result equal to constructor or one of those dynamic-code names fails. Runtime imports or re-exports of vm, node:vm, module, or node:module also fail. These closed syntactic rules deliberately reject the admitted direct, indirect, property-based, and constructor-chain source-text evaluation seams without claiming arbitrary JavaScript semantic proof. Every resolved local module is present exactly once, and every non-entry source module must be reachable; omission, addition, alias duplication, substituted bytes, or an import edge unequal to runtime_dependency_paths fails. This necessarily binds the Task 3 checkpoint/Unicode implementation and its core canonical-record dependencies whenever the Task 4 entry imports them. No constructor consults the live filesystem to complete this closure.
...
For each binding, the admissible CandidateRuleSet source closure is derived independently as the canonical union of every applicable acceptance-criteria snapshot's outer source_refs and every targeted policy snapshot's outer source_refs plus StableSetPayload.item_refs. Every source_refs member of every declared CandidateRuleSet must occur in that exact governance closure. General checkpoint reachability is insufficient: a ref reachable only from candidate A, candidate B, context, task, or fact material cannot authorize a rule source unless it also occurs in the independently derived acceptance-criteria/policy closure. This equation is enforced for every binding, including one unused by later vectorization.
...
An expression yielding zero word tokens is hard-ineligible and emits no vector. Every nonempty valid-scalar expression must yield at least one extended grapheme cluster; a zero-cluster result is a Unicode-runtime integrity failure, not an ineligible candidate state.
...
The aggregate hard-rule status is fail when any gate has hard_rule_status fail or the acceptance-criteria grapheme intersection is conflicting, and otherwise pass. The aggregate prohibited-claim status is hit when any gate has prohibited_claim_status hit and otherwise clear. Eligibility precedence uses those aggregates: quarantine, aggregate hard-rule fail, aggregate prohibited-claim hit, then empty word tokens. Thus a hard failure in any bound rule set or a contradictory checkpoint-bound length range wins even if every other set is clear, and a caller cannot obtain a vector by presenting only a permissive subset. An empty expression emits zero word tokens and therefore selects empty_word_tokens. Every nonempty valid-scalar expression has at least one extended grapheme cluster; if the supplied verified Unicode runtime reports otherwise, vectorization fails unicode_runtime and emits no ineligible result.
...
sha256Canonical({
  ...
  checkpoint_set_ref,
  feature_profile_ref,
  feature_profile_construction_input_digest,
  feature_universe_ref,
  feature_universe_artifact_ref,
  global_scope_material_refs,
  feature_materials,
...
feature_profile_construction_input_digest is sha256Canonical(profile_input) after the complete createFeatureProfile() replay succeeds.
...
vectorizeCandidate() calls createFeatureProfile(profile_input), requires the replayed FeatureProfile to equal profile byte-for-byte, and requires every overlapping Unicode-runtime, feature-universe, universe-artifact, scope-material, feature-material, hard-rule, checkpoint, lexicon, and runtime binding to be the exact value carried by profile_input. A self-hashed profile, a partial proof, or a replay proof for a different global or unused binding fails feature_profile_binding.

createDeterministicBaseline() validates the dependency-closed deterministic-baseline producer, calls createFeatureProfile(feature_profile_input), requires the replayed complete active development-fixture FeatureProfile to equal feature_profile byte-for-byte, and verifies the complete Task 3 StoreArtifactWitness for the runtime profile. The profile replay rechecks its complete feature universe, every checkpoint and binding including unused bindings, acceptance criteria, rule sets, materials, Unicode tables/runtime aggregate, generic lexicon, artifacts, producer, identity, digest, and provenance. The constructor reverifies the exact frozen runtime-profile path, UTF-8 bytes, raw digest, semantic object, Node 24.14.0 value, and ArtifactRef. The derived runtime ArtifactRef equals feature_profile.payload.runtime_profile_ref and the profile's exact runtime-artifact provenance projection. baseline.project_id is derived only from feature_profile.scope.project_id, baseline.feature_profile_ref is derived from the complete replayed profile, and baseline.runtime_profile_ref is derived from the verified witness. Candidate, selected target/candidate objects, gate results, and vectorization results remain absent from CreateBaselineInput. Supplying any extra field violates the closed input shape.
...
All three scoring functions first call createDeterministicBaseline(baseline_input), require the replayed baseline to equal baseline byte-for-byte, and require baseline_input.feature_profile to equal the separately supplied FeatureProfile. For every vector they then call vectorizeCandidate() with its complete paired vectorization_input, require an eligible result, and require the returned CandidateFeatureVector to equal the supplied vector byte-for-byte. This scoring-time replay rechecks the dependency manifest, complete profile construction proof, feature-universe artifact, profile-bound global scope-source ref union, selected target/candidate source byte equality, candidate and selected-binding material/acceptance-criteria/checkpoint bytes, the independently enumerated complete selected-binding policy-item rule-set collection, the manifest/profile global-union equations, every local gate and aggregate, effective grapheme intersection, Unicode/runtime behavior, exact derived values, counts, satisfied refs, provenance, identity, digest, and ref; a self-consistent caller-authored baseline, profile, or vector is insufficient. baseline.feature_profile_ref equals the exact supplied profile ref. baseline.project_id equals the replayed vector project, profile scope project, universe project, selected binding project, both scope projects, and checkpoint store project. baseline.runtime_profile_ref equals the replayed vector, profile payload, universe, selected binding, Unicode-runtime StoreArtifactWitness, and baseline runtime_artifact provenance refs. The replay also requires target_scope_role target. Universe, context, checkpoint, and Unicode-runtime equality are established from the complete paired vectorization input/result and, for pair or ordering operations, across every replayed candidate; they are not baseline fields. Each vector.feature_order equals, position for position, profile.payload.features ordered by position and therefore equals the exact 21-name tuple in section 7.3. values has exactly 21 finite entries; boolean and missing-indicator positions are exactly 0 or 1, numeric positions obey their declared [0,1] and missing-value rules. A mismatch fails before scoring as feature_profile_binding.
...
Pair vectors must be distinct by both candidate ref and vector ref and bind the same project, profile, context_ref, target_scope_ref/target role, feature_universe_ref, checkpoint_set_ref, runtime_profile_ref, and Unicode-runtime digest. Each vector equals the eligible replay of its paired vectorization input. BaselinePairResult binds both exact vector refs. Let d = score_a - score_b. Compute m as 1 / (1 + exp(-4 * abs(d))). Before clipping, probability A is m when d > 0, 1 - m when d < 0, and exactly 0.5 when d is exact binary64 zero. Probability A is clip of that value to [0.000001, 0.999999], and probability B is exactly 1 - probability A. This absolute-difference evaluation makes A/B reversal and complement exact under the frozen binary64 operation order. Valid replayed scores are in [0,1], so d is in [-1,1] and true exponential overflow or activation of either clip boundary is unreachable; the clip remains an identity-bound defensive postcondition and implementations must not bypass vector replay to manufacture an overflow or clipping test. Pair digest hashes every field except itself.
...
3. Only for development_fixture, perform one complete descriptor-only closed-shape/domain traversal of the entire explicit input graph before any producer, Unicode, digest, binding, quarantine, forbidden-key, or numeric work. This stage includes every nested replay input and its checkpoints, snapshots, events, receipts, artifacts, manifests, materials, rules, profiles, vectors, baselines, and candidate collections. It never invokes an accessor. After that whole-graph structural stage succeeds, validate categories globally in the exact code order below: every producer witness in the graph; every Unicode bundle/runtime; every claimed digest and derived ID; reference equality; scope; checkpoint closure; feature-profile replay; provenance; quarantine-expression absence; forbidden keys; and numeric finiteness. Operation order or parent-before-child traversal cannot allow a later category to hide an earlier one.

The digest category is a type- and path-complete graph stage, not a suffix-only scan. It verifies raw UTF-8 artifacts and store-artifact witnesses in their owning contexts; Task 2 snapshot envelopes and kind-specific inner expression or set digests; durable records by schema; Task 3 store bindings, manifest entries, stored events including prefix-only events, manifests, stream prefixes, receipts, and checkpoint sets; and Task 4 universe, scope-material, feature-material, rule-set, eligibility-gate, profile, vector, and baseline identities and content digests. Producer and Unicode artifact equations retain their earlier producer_witness and unicode_runtime ownership. Copy fields and refs remain later binding checks. A digest preimage containing a nonfinite number is not canonicalized; independent finite digest nodes are still checked, then the dedicated numeric_nonfinite stage reports that number.
...
Together the three suites exercise all eight public APIs with dual-fault tables: a complete nested canonical/closed-shape failure wins over a separate producer failure, and a producer failure wins over each later category reachable through that API, including stale supplied snapshot/baseline digests, quarantine/forbidden evidence, and numeric faults. The tables use complete valid fixtures before applying the two independent mutations.
...
- zero-token, hard-rule, prohibited-claim, and bounded quarantine negative results emit no vector; a nonempty valid-scalar expression producing zero grapheme clusters fails Unicode-runtime integrity and cannot emit a candidate result;
...
- vectorization replays the complete profile_input, binds its digest into the vector input preimage, and rejects a rehashed self-consistent profile, a different unused binding, or any unequal overlapping construction input;
...
- construction replays the complete feature_profile_input, requires byte equality with feature_profile, verifies the complete frozen runtime-profile StoreArtifactWitness and dependency-closed producer, and rejects a self-consistent forged profile or any extra construction field;
...
- exact tie 0.5, exact A/B reversal and probability complement, positive and negative evaluation at the valid d endpoints +1 and -1, the declared clip bounds, every emitted probability within those bounds, and score digest; tests must not bypass vector replay to force unreachable overflow or clipping;
...
- score, pair, and ordering replay the complete baseline_input plus every paired vectorization input and reject a digest-valid self-authored baseline/profile/vector or any replay that differs in profile-bound scope-source union/selected-source bytes, values, counts, satisfied refs, provenance, selected-binding material/acceptance-criteria enumeration, selected-binding policy-item rule sets, manifest/profile global-union equations, grapheme intersection, gates, or dependencies;
