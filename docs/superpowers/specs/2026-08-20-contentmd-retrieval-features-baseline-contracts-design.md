...
Artifact paths are workspace-relative POSIX paths. Raw digest is sha256 over exact UTF-8 bytes. resolution_artifacts is the exact path-sorted set package.json, packages/core/package.json, packages/core/tsconfig.json, packages/learning/package.json, packages/learning/tsconfig.json, pnpm-lock.yaml, tsconfig.base.json, and tsconfig.json. packages/learning/package.json declares exactly `"typescript-compiler": "npm:typescript@5.9.3"`; pnpm-lock.yaml resolves that alias to TypeScript 5.9.3. Those supplied, raw-byte-bound resolution artifacts make the compiler identity part of the producer witness. The compiler package is the trusted parser/resolver implementation and terminates dependency traversal like a Node built-in; it is not silently added to source_artifacts.
