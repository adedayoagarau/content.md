...
- `WritingBenchmarkTaskRecord`: `benchmark_id: "LIL-WRITE-001"`; `product_id: string`; `domain: string`; `channel: "web" | "notification"`; `locale: string`; `task_type: "strategy" | "contextual_microcopy"`; `family_id: string`; `task_packet_ref: Ref`; `context_evidence_ref: Ref`; `semantic_lineage_ids: string[]`; `template_ids: string[]`; `leakage_group_ids: string[]`; `required_facts: string[]` (empty allowed); `required_actions: string[]` (empty allowed); `acceptance_criteria: string[]`; `ownership: "project_owned_synthetic"`; `generation_config_ref: Ref`; `task_state`. The manifest is the sole owner of the exact 60 task refs; tasks bind the fixed benchmark identity rather than the manifest digest so membership remains one-way and digest construction is acyclic.
