...
The three optional snapshot kinds use only the closed `ExpressionFreeFeaturePayload`; they do not accept `unknown`, an extension object, free-form criteria, source excerpts, expression text, expression digests, labels, rationales, or browser/competitor/third-party values. The payload role and outer snapshot kind mapping is exactly `retrieval_snapshot -> retrieval`, `approved_pattern -> approved-pattern`, and `acceptance_criteria -> acceptance-criteria`. `permission_snapshot_ref` and `eligibility_checks_snapshot_ref` are non-authorizing, pre-presentation product-policy authority witnesses selected before the feature-source checkpoint is sealed and committed byte-for-byte by the optional snapshot envelope. They are complete well-formed refs, occur in the snapshot's exact `source_refs`, and remain independent from the later learning example's `eligibility_input.permission` and `eligibility_input.checks`. They MUST NOT be required to equal those later snapshots: the later checks bind the qualification and presentation, the presentation binds the checkpoint, and the checkpoint binds this payload, so equality would create an unconstructible cryptographic cycle. Independently, the later learning permission remains mandatory, current, and effective under section 7.1, its project list contains the payload project, and the later Task 2 checks snapshot is digest-valid with rights state `pass`. The frozen product-policy witnesses cannot authorize learning, and the later learning permission cannot rewrite, approve, or publish the checkpointed policy source.

`ordered_feature_refs` is nonempty, unique, and order-significant. Every ref resolves byte-identically to a different entry in the same feature manifest; self-reference and role cycles are impossible under the table's role graph. For an `approved_pattern`, every target is an active `approved_exemplar` whose `exemplar_kind` is `mechanism`. For a `retrieval_snapshot`, each target is either such an exemplar or an `approved_pattern` snapshot that recursively resolves only to such exemplars. For `acceptance_criteria`, every target is the one `fact_set` or `policy` entry, whose stable set is current. Define the payload's `resolved_subject_refs` as the unique sorted union of each target exemplar's `payload.subject_ref`, recursively through an approved-pattern target. Define `resolved_rights_refs` as the corresponding unique sorted union of each exemplar's `payload.rights_ref`. Every resolved exemplar subject ref occurs in the later learning permission payload's `subject_refs`, and every resolved exemplar rights ref occurs in the later eligibility rights check's `evidence_refs`. This later coverage replay is required in addition to, and never substituted for, the independently frozen product-policy authority refs. A fact-set or policy StableSet's item refs are product-governance inputs, not learning subjects or learning-rights evidence: they are validated by the checkpoint's complete current same-project source closure, its `training_permitted` disposition, and the optional snapshot's frozen product-policy authority refs, and MUST NOT be inserted into the later learning permission's subject or rights sets. They may govern eligibility or feature computation but cannot become labels, learned expressions, approvals, or publication authority.
