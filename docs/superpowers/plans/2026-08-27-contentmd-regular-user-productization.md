...
- the npm distribution verifier now clean-installs the four-file package, runs the installed no-argument scan, runs local `npx --no-install contentmd scan --summary`, verifies qualified counts and `write_effect: none`, confirms discovery state remains absent, and verifies clean uninstall;
