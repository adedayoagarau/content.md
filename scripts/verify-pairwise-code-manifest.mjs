...
const MANIFEST_PATH = "fixtures/learning-ranking/pairwise-training-code-manifest.json";
const RELEASE_PROFILE_PATH = "packages/learning/src/pairwise-release-profile.ts";
...
function fail(detail) {
  throw new Error(`pairwise code manifest invalid: ${detail}`);
}
...
if (!manifestBytes.equals(Buffer.from(canonicalJson(manifest), "utf8"))) fail("manifest bytes are not canonical");
...
let runtimePins;
try {
  runtimePins = runtimePinsText === undefined ? null : JSON.parse(runtimePinsText);
} catch {
...
if (rawPin !== manifestDigest || semanticPin !== manifestDigest
  ...
  fail("release profile pins");
}

process.stdout.write(`pairwise code manifest verified: ${manifest.entries.length} entries\n`);
