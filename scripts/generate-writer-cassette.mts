...
import { sha256Canonical } from "../packages/core/src/index.ts";
import {
  modelRequestDigest,
  type ModelProvider,
...
class CapturingProvider implements ModelProvider {
  ...
  async generate(request: ModelRequest): Promise<ModelResponse> {
    ...
    return {
      ...
      output_digest: sha256Canonical(output),
      output,
      token_accounting: { input_tokens: 100 + this.#index * 10, output_tokens: 50 + this.#index * 10 },
      deterministic_status: "recorded_exact",
...
await proposeContentRewrite(provider, { task, strategy, draft });
