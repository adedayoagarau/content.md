...
test("scores exact agreement and reports every required slice", () => {
  ...
  assert.equal(report.contract_version, "contentmd.content-design-evaluation-report/0.5.0");
  ...
  assert.equal(report.by_quality_dimension.accessibility_readiness.prediction_coverage, 1);
  assert.equal(report.by_quality_dimension.voice_fit.mean_absolute_error, 0);
  assert.equal(report.by_hard_dimension.factual_accuracy.accuracy, 1);
  assert.equal(report.by_hard_dimension.recovery.comparison_coverage, 1);
