...
describe("Task 6 no-influence shadow", () => {
  ...
  it("records one chronological observation but returns active-baseline output only", () => {
    ...
    const pair = source.replay.test_pairs[0]!;
    const candidates = [
      verifyPairwiseCandidate({
        record_mode: "development_fixture",
    ...
    const response = observeShadowSimulation(observationInput);
    ...
    const second = observeShadowSimulation({
      ...observationInput,
  ...
  it("fails closed when completion is claimed but terminal append crashes", () => {
    const { vault, sealedTest } = faultVault(
      shared.source.replay,
      "vault.task6.shadow-terminal-before-append",
      [{ operation: "shadow_terminal_append", occurrence: 1, fault: "before_append" }],
    );
    const plan = createShadowEvaluationPlan({
      record_mode: "development_fixture",
      sealed_test: sealedTest,
